...
class NodeManagerImplTest {
    ...
    @Test
    fun `established same-key duplicate deletes stale row preserves canonical without upsert`() {
        val oldNum = 1000
        ...
        verifySuspend(mode = VerifyMode.exactly(1)) { nodeRepository.reconcileIdentity(listOf(oldNum), null) }
    ...
    fun `different-key conflict preserves both nodes and emits no side effects`() {
        ...
        verifySuspend(mode = VerifyMode.not) { nodeRepository.reconcileIdentity(any(), any()) }
        verify(mode = VerifyMode.not) { notificationManager.dispatch(any()) }
    }
    ...
    fun `local authoritative renumber removes old same-key entries in memory upserts local no notification`() {
        ...
        verifySuspend(mode = VerifyMode.exactly(1)) { nodeRepository.reconcileIdentity(any(), any()) }
        verify(mode = VerifyMode.not) { notificationManager.dispatch(any()) }
    }
    ...
    // 9. Stale duplicate now issues a durable delete so the row cannot return after restart.

    @Test
    fun `stale duplicate triggers reconcileIdentity with delete for stale number`() {
        val oldNum = 1000
        val newNum = validPk.canonicalNum()
        ...
        assertNull(nodeManager.nodeDBbyNodeNum[oldNum])
        // Durable delete reached the repository for the stale packet.
        verifySuspend(mode = VerifyMode.exactly(1)) { nodeRepository.reconcileIdentity(any(), any()) }
    }
    ...
    // 26. Durable stale-row deletion prevents stale identity from returning after process restart.

    @Test
    fun `durable delete of stale identity prevents return after process restart`() {
        val key = ByteArray(32) { (it + 37).toByte() }.toByteString()
        val canonicalNum = key.canonicalNum()
        val staleNum = 9999 // arbitrary noncanonical num

        // Seed canonical and stale persisted rows via the in-memory index.
        nodeManager.updateNode(canonicalNum) { makeKnownNode(canonicalNum, key, "Canonical") }
        nodeManager.updateNode(staleNum) {
            Node(
                num = staleNum,
                user =
                User(
                    id = NodeAddress.numToDefaultId(staleNum),
                    long_name = "Meshtastic ${NodeAddress.numToDefaultId(staleNum).takeLast(4)}",
                    short_name = NodeAddress.numToDefaultId(staleNum).takeLast(4),
                    hw_model = HardwareModel.UNSET,
                ),
                publicKey = ByteString.EMPTY,
            )
        ...
        // Process the stale replay — incoming packet at staleNum with the key that belongs to canonicalNum.
        val capturedDeleteNums = mutableListOf<List<Int>>()
        val capturedUpsert = mutableListOf<Node?>()

        everySuspend { nodeRepository.reconcileIdentity(capture(capturedDeleteNums), capture(capturedUpsert)) } returns
            Unit
        ...
        // The stale number is removed from the in-memory index.
        assertNull(nodeManager.nodeDBbyNodeNum[staleNum])
        assertNotNull(nodeManager.nodeDBbyNodeNum[canonicalNum])

        // reconcileIdentity was called exactly once with the stale number to delete, no upsert.
        verifySuspend(mode = VerifyMode.exactly(1)) { nodeRepository.reconcileIdentity(any(), any()) }
        assertEquals(listOf(staleNum), capturedDeleteNums.first())
        assertNull(capturedUpsert.first())
        // No notification for a stale reconciliation.
        verify(mode = VerifyMode.not) { notificationManager.dispatch(any()) }
    }
