...
class NodeManagerImpl(
    ...
    private fun launchSessionWork(session: RadioSessionContext?, block: suspend () -> Unit) = if (session == null) {
    ...
    private fun updateNodeAndSchedulePersistence(
        ...
        if (shouldPersist(change.next)) launchSessionWork(session) { persistLatestNode(nodeNum) }
    }
    ...
    override fun insertMetadata(nodeNum: Int, metadata: DeviceMetadata, session: RadioSessionContext?) {
        launchSessionWork(session) { nodeRepository.insertMetadata(nodeNum, metadata) }
    }
