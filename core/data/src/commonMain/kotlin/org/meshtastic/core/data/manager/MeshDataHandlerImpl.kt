...
class MeshDataHandlerImpl(
    ...
    private fun launchSessionWork(session: RadioSessionContext?, block: suspend () -> Unit): Job =
        if (session == null) {
            scope.handledLaunch { block() }
        } else {
            // Acquire the nested lease before the caller releases its packet lease, so teardown waits for the
            // deferred side effect instead of racing its first dispatcher turn.
            scope.handledLaunch(start = CoroutineStart.UNDISPATCHED) {
                radioInterfaceService.runWithSessionLease(session) { block() }
            }
        }
    ...
    override fun handleReceivedData(
        ...
        collectorRegistry.collector?.let { collector ->
            if (collector.isActive) {
                launchSessionWork(session) { collector.onPacketReceived(packet, dataPacket) }
            }
        }
    }
    ...
    private fun rememberReaction(packet: MeshPacket, session: RadioSessionContext) = launchSessionWork(session) {
