...
class FromRadioPacketHandlerImpl(
    ...
    private fun handleClientNotification(cn: ClientNotification, session: RadioSessionContext) {
        ...
        scope.handledLaunch { dispatchClientNotification(cn, session) }
    }

    private suspend fun dispatchClientNotification(cn: ClientNotification, session: RadioSessionContext) {
        ...
        if (!dispatched) Logger.d { "Skipping client alert from stale transport session" }
    }
}
