...
private class SerialPortState {
    ...
    fun beginClose(): Job? = synchronized(lock) {
        closing = true
        startJob.also { startJob = null }?.also { it.cancel() }
    }
...
private fun diagnoseSerialOpenFailure(portName: String): String {
    ...
    return when {
        ...
        !portFile.canRead() || !portFile.canWrite() -> {
            ...
            "Permission denied for $devPath. Run: sudo usermod -aG $group $user  — then log out and back in."
        }
...
private constructor(
    ...
    override fun handleSendToRadio(p: ByteArray): Boolean {
        ...
        return if (port == null) {
            ...
            try {
                ...
                releaseOwnedByQueue = accepted
                accepted
            } finally {
                if (!releaseOwnedByQueue) lease.release()
            }
        }
    }
    ...
    override suspend fun close() {
        ...
        check(
            ...
            "[$portName] JVM serial teardown did not complete within its lifecycle bounds"
        }
    }
