...
class SerialRadioTransport(
    ...
    override suspend fun close() {
        ...
        check(completed) { "[$address] Serial teardown did not complete within its lifecycle bounds" }
    }
    ...
    private suspend fun closeClaimedConnection(claimed: ClaimedConnection, waitForStopped: Boolean) {
        ...
        check(completed) { "[$address] Serial connection teardown did not complete within its lifecycle bounds" }
    }
    ...
    override fun connect() {
        when {
            ...
            hasActiveConnection() -> Logger.d { "[$address] Ignoring start while serial generation is active" }
                ...
                val device = serialDevices.value[address]
                if (device == null) {
