...
class ReplayRadioTransport(
    ...
    override fun start() {
        lifecycle.runIfOpen {
            ...
            handshakeJob = transportScope.handledLaunch { for (nonce in handshakeQueue) replayHandshake(nonce) }
            Logger.i {
    ...
    override fun handleSendToRadio(p: ByteArray): Boolean = lifecycle.runIfOpen {
        ...
        when (wantConfigId) {
            ...
            ->
                handshakeJob?.isActive == true && handshakeQueue.trySend(wantConfigId).isSuccess
