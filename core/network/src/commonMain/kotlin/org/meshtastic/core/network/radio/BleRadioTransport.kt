...
private val BLE_TEARDOWN_TIMEOUT = 75.seconds
...
class BleRadioTransport(
    ...
    private suspend fun attemptConnection(): BleReconnectPolicy.Outcome {
        ...
        if (connectedReached == null) {
            ...
            withContext(NonCancellable) {
                ...
                if (retired != null) {
                    val sessionClosed = retired.lifecycle.close()
                    if (!sessionClosed) Logger.w { "[$address] Timed out retiring failed BLE profile generation" }
                    awaitPendingSessionCleanup()
                    disconnectGatt("Connected-gate timeout cleanup")
                } else {
                    awaitPendingSessionCleanup()
                }
            }
    ...
    private suspend fun discoverServicesAndSetupCharacteristics(): BleSession {
        ...
        try {
            bleConnection.profile(serviceUuid = SERVICE_UUID) { service ->
                ...
                // is retained.
                if (!sessionFailed.value && activeSession.value === session && !lifecycle.isClosed) {
    ...
    private suspend fun cleanupProfileSetupFailure(phase: String, expectedSession: BleSession?) {
        ...
        if (retired != null) {
            val completed = retired.lifecycle.close()
        ...
        try {
            completed =
                lifecycle.close {
                    ...
                    check(sessionClosed) {
                        "[$address] BLE profile teardown did not complete within its lifecycle bounds"
                    }
                }
            check(completed) { "[$address] BLE teardown did not complete within its lifecycle bounds" }
        } finally {
            if (!completed) {
