...
interface RadioSessionAuthority {
    ...
    /** Returns whether [session] still owns transport admission. */
    fun isSessionActive(session: RadioSessionContext): Boolean = activeSession.value == session

    /**
     * Runs [block] only while [session] owns transport admission. Implementations with a revocation lock should
     * override this so the check and synchronous side effect are atomic with teardown.
     */
     ...
     * teardown has closed new admission. Callers must keep the block bounded and must not invoke transport lifecycle
    ...
    suspend fun runWithSessionLease(session: RadioSessionContext, block: suspend (RadioSessionLease) -> Unit): Boolean {
        ...
        block(
            object : RadioSessionLease {
                ...
                override fun isCurrent(): Boolean = isSessionActive(session)
            },
        )
        return true
    }
