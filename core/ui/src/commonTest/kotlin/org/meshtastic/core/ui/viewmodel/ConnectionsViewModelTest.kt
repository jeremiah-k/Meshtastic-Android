...
class ConnectionsViewModelTest {
    ...
    fun `Disconnected with Reconnecting progress maps to RECONNECTING`() = runTest {
        viewModel.connectionStatus.test {
            ...
            serviceRepository.setConnectionProgress(RECONNECTING_PROGRESS_TEXT)
     ...
     * NOT_CONNECTED instead of RECONNECTING. This test pins the constant; combined with the existing ordering test in
     * MeshConnectionManagerImplTest that asserts the literal "Reconnecting…" is set, this transitively enforces the
     * contract without extracting a shared constant across modules.
     */
    @Test
    fun `RECONNECTING_PROGRESS_TEXT pins the cross-track literal value`() {
        assertEquals("Reconnecting\u2026", RECONNECTING_PROGRESS_TEXT)
    }
}
