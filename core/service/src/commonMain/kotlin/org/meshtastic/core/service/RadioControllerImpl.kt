...
class RadioControllerImpl(
            ...
            combine(
                ...
                .collectLatest { snapshot ->
                    ...
                    if (
                        ...
                        snapshot.activeSession?.address != identity.address ||
                        snapshot.activeSession?.generation != identity.sessionGeneration
                    ) {
    ...
    override suspend fun setDeviceAddress(address: String) {
        deviceSwitchMutex.withLock {
            ...
            val previousAddress = radioInterfaceService.getDeviceAddress() ?: meshPrefs.deviceAddress.value
            if (address == previousAddress) {
                // Repair a lagging preference flow even when the transport is already on the requested address.
