...
class LegacyDfuTransportTest {
    ...
    class LegacyResponder {
        ...
        /**
         * Optional hook invoked after each firmware-packet write with the cumulative byte count. Tests use this to
         * simulate mid-stream link drops at a deterministic offset.
         */
        var onFirmwarePacketWrite: ((bytesReceived: Long) -> Unit)? = null
    ...
    fun `mid-stream disconnect returns typed MidStreamDisconnect with correct byte counts`() = runTest {
        ...
        // Trigger a remote disconnect after the first firmware packet write (20 bytes at MTU=20). The disconnect
        // tripwire watches the connection state and surfaces a MidStreamDisconnect carrying the in-flight offset.
        var disconnected = false
        env.responder.onFirmwarePacketWrite = { bytes ->
            if (bytes >= 20L && !disconnected) {
                disconnected = true
        ...
        // First packet landed (20B); the disconnect fired before the second write, so bytesSent == 20.
        assertEquals(20, ex.bytesSent)
        assertEquals(200, ex.totalBytes)
        // No PRN was received before the drop (first PRN would be at byte 200), so lastConfirmedBytes is -1.
        assertEquals(-1, ex.lastConfirmedBytes)
        assertTrue(ex.connectionState.isNotBlank())
    }
