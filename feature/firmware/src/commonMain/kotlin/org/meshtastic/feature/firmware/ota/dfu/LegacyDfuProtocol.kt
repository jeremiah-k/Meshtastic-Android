...
sealed class LegacyDfuException(message: String, cause: Throwable? = null) : DfuException(message, cause) {
    ...
    /**
     * The BLE link dropped while firmware bytes were in flight. Carries the host-sent offset (the number of bytes
     * written via WITHOUT_RESPONSE up to the drop) so the retry coordinator can distinguish a mid-stream failure (which
     * switches subsequent Legacy uploads to [LegacyDfuStreamProfile.RECOVERY]) from a handshake/connect failure (which
     * does not).
     *
     * [bytesSent] reflects host-sent progress — WITHOUT_RESPONSE writes accepted or queued by the host stack — and is
     * NOT confirmed device progress. [lastConfirmedBytes] carries the last PRN-confirmed offset (the last byte the
     * bootloader explicitly acknowledged); `-1` means no PRN was received before the drop. Do not treat [bytesSent] as
     * confirmed device progress.
     *
    ...
    class MidStreamDisconnect(
        ...
        "BLE link dropped mid-upload at host-sent byte $bytesSent/$totalBytes " +
